...
near(measurement_mean, 3.333333, 1e-6)
```
...
ymd_hm("2022-11-18 12:00") + my_datesdiff
...
new_data_selection <- gbd_short %>% 
  filter((year == 1990 | year == 2017) & cause == "Communicable diseases")
...
gbd_full %>%
  filter(year == 1990) %>%
  group_by(income, sex) %>%
  summarise(deaths = sum(deaths_millions)) %>%
  pivot_wider(names_from = income, values_from = deaths)
```
...
# Since the data seems to skew a little on both ends of the
# QQ plot, it looks like it is not normally distributed.
```

### 6.12.2 Exercise

```{r}
paired_data_afr <- gapdata %>%
  filter(year %in% c(2002, 2007)) %>%
  filter(continent == "Africa")

fig1 <- paired_data_afr %>%
  ggplot(aes(x = lifeExp)) +
  geom_histogram(bins = 12)

fig2 <- paired_data_afr %>%
  ggplot(aes(sample = lifeExp)) +
  geom_qq() +
  geom_qq_line(colour = "blue")

fig3 <- paired_data_afr %>%
  ggplot(aes(x = year, y = lifeExp)) +
  geom_boxplot(aes(fill = factor(year))) + 
  geom_jitter(alpha = 0.5)
...
fit = aov(lifeExp ~ continent, data = anov_data) 
summary(fit)
```
...
meldata %>% 
  summary_factorlist(dependent = "status.factor", 
                     explanatory = 
                       c("sex.factor", "ulcer.factor", "age.factor", "thickness"),
                     cont = "median"
  )
```
...
```{r}
meldata %>%
  count(sex.factor, age.factor, status.factor) %>%
  group_by(status.factor) %>%
  mutate(total = sum(n)) %>%
  mutate(percentage = round(100*n/total, 1)) %>% 
  mutate(count_perc = paste0(n, " (", percentage, ")")) %>% 
